...
from weproject.models import User, Post
from flask_login import current_user
...
class RegistrationForm(FlaskForm):
    ...
    my_token = StringField('Token', validators=[DataRequired(), Length(min=5, max=20, message=('Please your token is invalid or Incorrect'))])
...
class PostForm(FlaskForm):
    ...
    title = StringField('Title', validators=[DataRequired(), Length(min=5, max=20, message=('Please we accept a minimum of 5 characters for project title'))])
    description = TextAreaField('Descrption', validators=[DataRequired(), Length(min=5,  message=('Please give details intro about the project*'))])
    ...
    filee = FileField('Upload application/ project', validators=[FileAllowed(['rar', 'zip', 'mp3', 'mp4'])])
    screenshot = FileField('Appication/ project image ', validators=[FileAllowed(['jpg', 'jpeg', 'png', 'jfif'])])
